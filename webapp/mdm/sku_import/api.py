"""Thin HTTP adapter for SKU Import V1 (mirrors Product Import V1)."""
...
from ..import_engine.sku_commit import SKUCommitService
from ..import_engine.sku_review import SKUReviewService
...
router = APIRouter(prefix="/api/v1/mdm/sku-import", tags=["sku-import"])
...
SKU_TEMPLATE_FILENAME = "sku_import_v1_template.xlsx"
...
def download_template(user=Depends(mdm_view)):
    return Response(
        content=build_import_template(SKU_TEMPLATE_SHEET, SKU_TEMPLATE_COLUMNS),
        media_type=XLSX_MEDIA_TYPE,
        headers={
            "Content-Disposition": f'attachment; filename="{SKU_TEMPLATE_FILENAME}"'
        },
    )


def _sku_batch(db: Session, batch_id: str):
    batch = get_batch(db, batch_id)
    if (
        enum_value(batch.import_mode) != ImportMode.OPERATIONAL.value
        or batch.entity_type != ImportType.SKU.value
    ):
        raise WorkflowError(
...
def _detail(db: Session, batch_id: str) -> BatchDetail:
    batch, rows, findings, _decisions = _sku_batch(db, batch_id)
    counts = _counts(rows)
...
def _review_payload(db: Session, batch_id: str) -> ReviewPayload:
    batch, rows, findings, decisions = _sku_batch(db, batch_id)
    counts = _counts(rows)
...
def _upload_result(db: Session, preview) -> UploadResult:
    batch, rows, findings, _decisions = _sku_batch(db, preview.batch_id)
    by_row: dict[int, list[ImportFinding]] = defaultdict(list)
    for finding in findings:
        by_row[finding.import_row_id].append(finding)
    counts = _counts(rows)
    return UploadResult(
        ...
        validation=ValidationResult(
            file_valid=True,
            sheet=batch.worksheet or SKU_TEMPLATE_SHEET,
            file_errors=[],
            counts=counts,
            commit_eligible=derive_commit_eligible(counts),
        ),
...
def upload(
    ...
    with tempfile.TemporaryDirectory(prefix="sku-import-") as tmpdir:
...
def list_batches(
    ...
    query = select(ImportBatch).where(
        ...
        ImportBatch.entity_type == ImportType.SKU.value,
    )
...
def acknowledge(
    ...
    workflow_response(lambda: _sku_batch(db, batch_id))
    result = workflow_response(
        lambda: SKUReviewService(db).acknowledge(
...
def commit(
    ...
    return workflow_response(
        lambda: SKUCommitService(db).commit(
