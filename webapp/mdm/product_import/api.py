"""Thin HTTP adapter for Product Import V1."""
...
from ..import_engine.product_commit import ProductCommitService
from ..import_engine.product_review import ProductReviewService
...
router = APIRouter(prefix="/api/v1/mdm/product-import", tags=["product-import"])
...
PRODUCT_TEMPLATE_FILENAME = "product_import_v1_template.xlsx"
...
def download_template(user=Depends(mdm_view)):
    return Response(
        content=build_import_template(PRODUCT_TEMPLATE_SHEET, PRODUCT_TEMPLATE_COLUMNS),
        media_type=XLSX_MEDIA_TYPE,
        headers={
            "Content-Disposition": (
                f'attachment; filename="{PRODUCT_TEMPLATE_FILENAME}"'
            )
        },
    )


def _product_batch(db: Session, batch_id: str):
    batch = get_batch(db, batch_id)
    if (
        enum_value(batch.import_mode) != ImportMode.OPERATIONAL.value
        or batch.entity_type != ImportType.PRODUCT.value
    ):
        raise WorkflowError(
...
def _existing_view(row, findings) -> ExistingView:
    ...
    return ExistingView(
        ...
        master_detail_url=f"/api/v1/mdm/products/{matched}" if matched else None,
    ...
    for row in rows:
        ...
        result.append(
            ReviewRow(
                ...
                status=row.status,
                identity={
                    "product_code": str(normalized.get("product_code") or ""),
                    "product_name": str(normalized.get("product_name") or ""),
                },
...
def _detail(db: Session, batch_id: str) -> BatchDetail:
    batch, rows, findings, _decisions = _product_batch(db, batch_id)
    counts = _counts(rows)
...
def _review_payload(db: Session, batch_id: str) -> ReviewPayload:
    batch, rows, findings, decisions = _product_batch(db, batch_id)
    counts = _counts(rows)
...
def _upload_result(db: Session, preview) -> UploadResult:
    batch, rows, findings, _decisions = _product_batch(db, preview.batch_id)
    by_row: dict[int, list[ImportFinding]] = defaultdict(list)
    for finding in findings:
        by_row[finding.import_row_id].append(finding)
    counts = _counts(rows)
    return UploadResult(
        ...
        validation=ValidationResult(
            file_valid=True,
            sheet=batch.worksheet or PRODUCT_TEMPLATE_SHEET,
            file_errors=[],
            counts=counts,
            commit_eligible=derive_commit_eligible(counts),
        ),
...
def upload(
    ...
    with tempfile.TemporaryDirectory(prefix="product-import-") as tmpdir:
        ...
        try:
            preview = ImportService(db).stage_file(
                ...
                or PRODUCT_IMPORT_SOURCE_SYSTEM,
                uploaded_by=user["id"],
            )
...
def list_batches(
    ...
    query = select(ImportBatch).where(
        ...
        ImportBatch.entity_type == ImportType.PRODUCT.value,
    )
...
def acknowledge(
    ...
    workflow_response(lambda: _product_batch(db, batch_id))
    result = workflow_response(
        lambda: ProductReviewService(db).acknowledge(
...
def commit(
    ...
    return workflow_response(
        lambda: ProductCommitService(db).commit(
